...
|:--|:--|
|a|Enter your home|
|b|List what is in your home and access one of directories you find with `cd name_directory`.
|c|Move one directory up with `cd ..`|
| |The `..` is a general way to say *one up*. You can concatenate the points, i.e., `cd ../..` means go **two** directories up, `cd ../../..` means **three** and so on. The single dot `.`, insted, indicates *this* directory where you are now. For example, running `cd .` moves you...nowhere, since you are telling the computer to move to where you are to where you are. While the single dot might seem (and it is) useless in the context of this command, it will be useful for others that you will encounter down the road.|
...
Try navigating using both absolute and relative path. Remember that you can always go back to your home with `cd` or `cd ~`.
...
|4|Creating files|
|:--|:--|
|a|Create three files: `dog.txt`, `cat.txt`, `elephant.txt` in the `animals` directory|
|b|Check that they exist using `ls`|
|c|What is their relative path? And their absolute one?|
|d|Go back two directories to the one containing `playground` with `cd ../..` and try to create `bamboo.txt` and `baobab.txt` inside `plants` **without** entering in `plants` but by using relative or absolute paths|
...
|5|Reading and echoing|
|:--|:--|
|a|Navigate to the directory containing the file `dog.txt`|
|b|Display the contents of `dog.txt` using `cat`. It should be empty.|
