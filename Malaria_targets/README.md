...
```
gmx trjconv -f prod.xtc -s nvt_1.tpr -pbc mol -center -o cleaned_traj.xtc
```
and, when prompted by GROMACS, by selecting the group `Protein` for centering and `non-Water` for output. The output will be called `cleaned_traj.xtc` and will contain the frames with the positions of the protein, the ligand (if any), and the ions (if any). However, the `nvt_1.tpr` file still contains the information of the whole system. You can create a new tpr that reflects the new content of your system with the following
```
gmx convert-tpr -s nvt_1.tpr -o cleaned_traj.tpr
```
